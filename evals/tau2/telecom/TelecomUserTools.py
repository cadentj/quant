"""Closely adapted from https://github.com/sierra-research/tau2-bench/blob/377cecff73b873e63430a0acfce00e0eff024855/src/tau2/domains/telecom/user_tools.py"""
...
from evals.tau2.telecom.user_data_model import (
    APNNames,
    APNSettings,
    AppPermissions,
    AppStatus,
...
from evals.tau2.utils import TAU2_DATA_DIR


class TelecomUserTools(StoreModel):
    """Provides methods to simulate user actions and agent instructions on a MockPhoneAttributes instance."""

    db: TelecomUserDB = Field(default_factory=TelecomUserDB)

    network_mode_preference: NetworkModePreference = Field(
        default=NetworkModePreference.FOUR_G_5G_PREFERRED
    )

    default_vpn_details: VpnDetails = Field(
        default=VpnDetails(
    ...
    def initialize(self) -> None:
        ...
        DB_FILE = TAU2_DATA_DIR / "domains" / "telecom" / "user_db.toml"
        setattr(self, "db", TelecomUserDB.load(str(DB_FILE)))
    ...
    def set_network_mode_preference(self, mode: NetworkModePreference | str) -> str:
    ...
    def _run_speed_test(self) -> tuple[float | None, str | None]:
        ...
        if simulated_speed < 1:  # noqa: PLR2004
            desc = "Very Poor"

        elif simulated_speed < 5:  # noqa: PLR2004
            desc = "Poor"
        elif simulated_speed < 25:  # noqa: PLR2004
            desc = "Fair"

        elif simulated_speed < 100:  # noqa: PLR2004
            desc = "Good"
        else:
            desc = "Excellent"
    ...
    def set_apn_settings(self, apn_settings: APNSettings | dict[str, Any]) -> str:
    ...
    def _reset_apn_settings(self) -> str:
    ...
    def set_wifi_calling(self, enabled: bool, mms_over_wifi: bool | None = None) -> str:
    ...
    def break_vpn(self) -> str:
        ...
        self.connect_vpn()
        if self.device.vpn_details is not None:

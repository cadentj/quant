"""Data model of the airline domain for TAU2. Closely adapted from https://github.com/sierra-research/tau2-bench/blob/377cecff73b873e63430a0acfce00e0eff024855/src/tau2/domains/airline/data_model.py"""

from typing import Annotated, Any, Literal

from pydantic import BaseModel, Field

from evals.tau2.environment.db import DB
...
class Address(BaseModel):
    ...
    address2: str | None = Field(None, description="Secondary address line (optional)")
    city: str = Field(description="City name")
...
class CreditCard(PaymentMethodBase):
    source: Literal["credit_card"] = Field(  # type: ignore
...
class GiftCard(PaymentMethodBase):
    source: Literal["gift_card"] = Field(  # type: ignore
...
class Certificate(PaymentMethodBase):
    source: Literal["certificate"] = Field(  # type: ignore
...
class FlightDB(DB):
    ...
    users: dict[str, User] = Field(
        default_factory=dict, description="Dictionary of all users indexed by user ID"
    )

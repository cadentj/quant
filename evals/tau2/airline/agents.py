from inspect_ai.agent import Agent

from evals.tau2.airline.tools import all_airline_tools
from evals.tau2.common.agents import (
    CustomerServiceAgentConfig,
    get_user_simulator_agent,
)
from evals.tau2.utils import TAU2_DATA_DIR


def get_domain_policy() -> str:
    policy_path = TAU2_DATA_DIR / "domains" / "airline" / "policy.txt"
    policy = policy_path.read_text(encoding="utf-8")
    return f"""<policy>
{policy}
</policy>"""


def get_user_agent(instance: str, message_limit: int | None = None) -> Agent:
    return get_user_simulator_agent(
        config=CustomerServiceAgentConfig(
            get_domain_policy=get_domain_policy,
            tools=[t() for t in all_airline_tools],
            instance=instance,
        ),
        message_limit=message_limit,
    )
